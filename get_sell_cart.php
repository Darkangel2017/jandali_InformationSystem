<thead>
	...
	$q="SELECT * FROM sell_cart WHERE user_id='{$_SESSION['user_id']}'";
	$cart_id=mysqli_fetch_assoc(mysqli_query($conn,$q))["cart_id"];
	$q="SELECT quantity as c,item_id,cart_id,items_in_cart_id FROM `items_in_sell_cart` WHERE cart_id='{$cart_id}'";
	...
	while($row1=mysqli_fetch_assoc($items_in_cart)){
		...
		echo "<td><button onclick='delete_items_from_cart_sell(".$row["item_id"].")' class='deletebutton'>Delete</button></td>";
        echo "</tr>";
        $final_price=$final_price+($row['selling_price']*$row1['c']);
	}
	?>
	<tfoot>
		<tr>
