...
$q="SELECT COUNT(items_in_cart_id) as count , item_id FROM `items_in_sell_cart` GROUP BY item_id ";
...
echo '<img src="'.$image_path.'" id="company_logo" style="width: 27em; height: 16em;" alt="Company Logo" class="icon">';
  ...
  echo ' <div class="container">
            ...
            <span>Quotation Date: </span><span id="invoice_date">'.$date.'</span><br>
            <span>Quotation Number: </span><span id="invoice_nb">'.$inv_number['items_in_cart_id'].'</span><br>
            </div>
            ...
            <span class>Paitent Name :</span><span id="payment_method">'.$paitent_name.'</span>
            </section>
            </div>';
  echo '<table id="tablePreview" class="table table-striped">
    ...
    while ($item=mysqli_fetch_assoc($items_in_cart)) {
        ...
        echo '<td></td>';
        echo  '<td>'.$item_row['ministry_code'].'</td>';
    ...
    $final_price_with_disc=$final_price;
